...
public class PlayerEggCatching : MonoBehaviour {
	...
	void InitializeData(){
		GameData.GAME_REQUIRED = 10;
		CatcherRigid2D = GetComponent<Rigidbody2D> ();	
	}
    ...
    void ClampPlayer(){
		CatcherPosition = CatcherRigid2D.position;
		CatcherPosition.x = Mathf.Clamp (CatcherPosition.x, minPlayerClampPosX, maxPlayerClampPosX);
		CatcherRigid2D.position = CatcherPosition;
	}

	public void MovePlayerLeft(){
		CatcherRigid2D.velocity = new Vector2 (-movementSpeed, 0);
	}
	...
	public void StopPlayerMovement(){
		CatcherRigid2D.velocity = Vector2.zero;
	}

	void OnTriggerEnter2D(Collider2D col){
		if (col.gameObject.tag == "EggGame_Egg") {
